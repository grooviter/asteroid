...
.annotation
----
include::{testMain}/asteroid/local/samples/NameChecker.groovy[]
----

NOTE: Most of the times a single node type is targeted. That
...
WARNING: The following example of method overriding only works with dynamic Groovy, if
...
.implementation
----
include::{testMain}/asteroid/local/samples/NameCheckerImpl.groovy[]
----
...
.example
----
include::{testTest}/asteroid/local/samples/NameCheckerTest.groovy[tags=checkerSucceeds, indent=0]
----
